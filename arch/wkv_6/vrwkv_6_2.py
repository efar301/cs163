...
class StarShift(nn.Module):
    def __init__(self, channels, n_head, head_dim):
        super().__init__()
        def dilation(factor):
        ...
        self.alpha = nn.Parameter(torch.zeros(n_head, 3))

    def forward(self, x, patch_resolution):
        B, T, C = x.shape
        Hh, Hd = self.n_head, self.head_dim
        H, W = patch_resolution
        feat = x.reshape(B, H, W, C).permute(0, 3, 1 ,2).contiguous()
        y1, y2, y3 = self.conv1(feat), self.conv2(feat), self.conv3(feat)
        y = torch.stack([y1, y2, y3], dim=2)
        y = y.view(B, Hh, Hd, 3, H, W)
        
        w = torch.softmax(self.alpha, dim=1).view(1, Hh, 1, 3, 1, 1)
        out = (y * w).sum(dim=3)
        out = out.view(B, C, H, W).permute(0, 2, 3, 1).reshape(B, T, C).contiguous()
        return out
