...
#             nn.Conv2d(channels, self.reduced, kernel_size=1, bias=False),
#             nn.SiLU(),
#             nn.Conv2d(self.reduced, channels, kernel_size=1, bias=False),
#         )
#         self.dilation_weights = nn.Parameter(torch.zeros(len(self.dilations)))

#     def forward(self, x, patch_resolution):
#         B, T, C = x.shape
#         H, W = patch_resolution
#         x2 = x.transpose(1, 2).reshape(B, C, H, W)
#         feats = torch.stack([b(x2) for b in self.branches], dim=1)  # [B, K, C, H, W]
#         w = torch.softmax(self.dilation_weights, dim=0).view(1, -1, 1, 1, 1)
#         y = (feats * w).sum(dim=1)  # [B, C, H, W]
#         y = self.pointwise(y)
...
class SpatialShift(nn.Module):
    def __init__(self, channels, shift_pixel=1):
        ...
        self.dconv3 = nn.Conv2d(channels, channels, kernel_size=3, padding=3, dilation=3, groups=channels, bias=False)

        # learnable weights for shifts
        self.dw1 = nn.Parameter(torch.rand(1, channels, 1, 1))
        self.dw2 = nn.Parameter(torch.rand(1, channels, 1, 1))
        self.dw3 = nn.Parameter(torch.rand(1, channels, 1, 1))
        self.qw = nn.Parameter(torch.rand(1, channels, 1, 1))


    def forward(self, x, patch_resolution=None):
        B, T, C = x.shape
        H, W = patch_resolution
        x1 = x.transpose(2, 1).reshape(B, C, H, W)
        d1 = self.dconv1(x1)
        d2 = self.dconv2(x1)
        d3 = self.dconv3(x1)

        q = q_shift_multihead(x, shift_pixel=self.shift_pixel, head_dim=HEAD_SIZE, patch_resolution=patch_resolution)
        q = q.transpose(2, 1).reshape(B, C, H, W)
        out = self.dw1 * d1 + self.dw2 * d2 + self.dw3 * d3 + self.qw * q
        return out.reshape(B, C, T).transpose(2, 1)
...
class HighFrequencyAggregator(nn.Module):
    ...
    def forward(self, x, patch_resolution=None):
        ...
        high = x2 - low
        high = high.reshape(B, C, T).transpose(1, 2)
        return high
...
class VRWKV_SpatialMix_V6(nn.Module):
    ...
    def jit_func(self, x, patch_resolution):
        ...
        xx = self.shift_func(x, patch_resolution=patch_resolution) - x
...
class VRWKV_ChannelMix(nn.Module):
    def __init__(self, n_embd, n_head, n_layer, layer_id, shift_mode='q_shift_multihead',
        ...
        # self.shift_func = eval(shift_mode)
        self.shift_func = SpatialShift(n_embd, shift_pixel)
        self.high_pass = HighFrequencyAggregator(n_embd)
        self.high_pass_weight = nn.Parameter(torch.rand(1, 1, n_embd))
    ...
    def forward(self, x, patch_resolution=None):
        def _inner_forward(x):
            ...
            xx_k = self.shift_func(hp, patch_resolution=patch_resolution)
            xx_r = self.shift_func(x, patch_resolution=patch_resolution)
            xk = hp * self.spatial_mix_k + xx_k * (1 - self.spatial_mix_k)
            xr = x * self.spatial_mix_r + xx_r * (1 - self.spatial_mix_r)
            # xx = self.shift_func(x, patch_resolution=patch_resolution)
