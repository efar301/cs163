...
class DilationShift(nn.Module): 
    def __init__(self, channels): 
        ...
        self.branches = nn.ModuleList(
            [ nn.Conv2d(channels, channels, kernel_size=3, padding=d, dilation=d, groups=channels, bias=False) for d in self.dilations]
        ) 
    ...
    def forward(self, x, patch_resolution): 
        ...
        y = sum(branch(x2) for branch in self.branches) / len(self.dilations) 
        y = y.reshape(B, C, T).transpose(1, 2) 
        return y

class SpecialShift(nn.Module):
    def __init__(self, channels, shift_pixel=1, head_dim=HEAD_SIZE):
        super().__init__()
        self.channels = channels
        self.shift_pixel = shift_pixel
        self.head_dim = head_dim

        self.dilation_shift = DilationShift(channels)
        self.q_shift_weight = nn.Parameter(torch.ones(1, 1, channels) * 0.5)
        self.dilation_shift_weight = nn.Parameter(torch.ones(1, 1, channels) * 0.5)

    def forward(self, x, patch_resolution=None):
        x1 = q_shift_multihead(x, self.shift_pixel, self.head_dim, patch_resolution)
        x2 = self.dilation_shift(x, patch_resolution)
        out = self.q_shift_weight * x1 + self.dilation_shift_weight * x2
        return out
...
class VRWKV_SpatialMix_V6(nn.Module):
    def __init__(self, n_embd, n_head, n_layer, layer_id, shift_mode='q_shift_multihead',
        ...
        self.shift_func = SpecialShift(n_embd, shift_pixel, self.head_size)
...
class VRWKV_ChannelMix(nn.Module):
    def __init__(self, n_embd, n_head, n_layer, layer_id, shift_mode='q_shift_multihead',
        ...
        self.high_pass_weight = nn.Parameter(torch.ones(1, 1, n_embd) * 0.1)
