...
class DilationShift(nn.Module):
    def __init__(self, channels):
        ...
        self.branches = nn.ModuleList([
            nn.Conv2d(channels, channels, kernel_size=3, padding=d*(3//2), dilation=d, groups=channels, bias=False)
...
class SpecialShift(nn.Module):
    def __init__(self, channels, shift_pixel=1, gamma=1/4):
        super().__init__()
        ...
        self.weight1 = nn.Parameter(torch.ones(1, 1, channels) * 0.5)
        self.weight2 = nn.Parameter(torch.ones(1, 1, channels) * 0.5)

    def forward(self, x, patch_resolution=None):
        x1 = q_shift(x, self.shift_pixel, self.gamma, patch_resolution)
        x2 = self.dilation_shift(x, patch_resolution)
        out = self.weight1 * x1 + self.weight2 * x2 
        return out
...
class VRWKV_SpatialMix(nn.Module):
    def __init__(self, n_embd, n_layer, layer_id, shift_mode='q_shift',
        ...
        self.channel_gamma = channel_gamma
        self.shift_func = SpecialShift(n_embd, shift_pixel, channel_gamma)
...
class VRWKV_ChannelMix(nn.Module):
    ...
    def forward(self, x, patch_resolution=None):
        ...
        if self.shift_pixel > 0:
            # xx = self.shift_func(x, self.shift_pixel, self.channel_gamma, patch_resolution)
            # xk = hp * self.spatial_mix_k + xx * (1 - self.spatial_mix_k)
            # xr = x * self.spatial_mix_r + xx * (1 - self.spatial_mix_r)
