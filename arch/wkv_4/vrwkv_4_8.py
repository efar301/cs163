...
class VRWKV_SpatialMix(nn.Module):
    ...
    def _init_weights(self, init_mode):
        ...
        if init_mode == 'fancy':
            with torch.no_grad():
                ...
                self.alphas = nn.Parameter(torch.tensor([0.5, 0.5]))
    ...
    def forward(self, x, patch_resolution=None):
        ...
        # row major scan
        v_h = RUN_CUDA(self.spatial_decay[0] / T, self.spatial_first[0] / T, k, v)

        # column major scan
        k_v = rearrange(k, 'b (h w) c -> b (w h) c', h=H, w=W)
        v_v = rearrange(v, 'b (h w) c -> b (w h) c', h=H, w=W)
        v_v = RUN_CUDA(self.spatial_decay[1] / T, self.spatial_first[1] / T, k_v, v_v)
        v_v = rearrange(v_v, 'b (w h) c -> b (h w) c', h=H, w=W)

        rwkv = self.alphas[0] * v_h + self.alphas[1] * v_v
